...
impl FlagsOps for u8 {
    ...
    fn set_flag(&mut self, f: Flag, value: bool) {
        match f {
            Flag::Zero      => { if value { *self |= (value as u8) << 7 } else { *self &= !(1u8 << 7) }}
            Flag::Subtract  => { if value { *self |= (value as u8) << 6 } else { *self &= !(1u8 << 6) }}
            Flag::HalfCarry => { if value { *self |= (value as u8) << 5 } else { *self &= !(1u8 << 5) }}
            Flag::Carry     => { if value { *self |= (value as u8) << 4 } else { *self &= !(1u8 << 4) }}
        }
    }
}
