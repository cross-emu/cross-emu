...
impl GameCT for GameCommunicationTool {
    fn update_input(&mut self, input_ref: &mut KeyInput) -> Result<(), String> {
        match self.input_receiver.has_changed() {
            Ok(true) => {
                *input_ref = *self.input_receiver.borrow_and_update();
                Ok(())
            }
            Ok(false) => Ok(()),
    ...
    fn update_fps(&mut self, fps: u128) -> Result<(), String> {
        self.fps
            .store(fps as isize, std::sync::atomic::Ordering::Relaxed);
        Ok(())
    }
    ...
    fn poll_requests(&mut self) -> Vec<Request> {
        ...
        while let Ok(request) = self
            .request_receiver
            .try_recv()
            .map_err(|err| format!("State of the request receiver : {:?}", err))
        {
